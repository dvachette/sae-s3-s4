  ...
  static fromRow(row) {
    ...
    for (const friendRow of friendsRows) {
      if (friendRow.status === 'accepted') {
        const friendId =
          friendRow.senderId === user.userId
            ? friendRow.receiverId
            : friendRow.senderId;
        const friendNameQuery = new Database('database.db').prepare(
    ...
    const deckCard1 =
      user.collection.find((item) => item.card.cardId === row.card1Id)?.card ||
      null;
    const deckCard2 =
      user.collection.find((item) => item.card.cardId === row.card2Id)?.card ||
      null;
    const deckCard3 =
      user.collection.find((item) => item.card.cardId === row.card3Id)?.card ||
      null;
    const deckCard4 =
      user.collection.find((item) => item.card.cardId === row.card4Id)?.card ||
      null;
    const deckCard5 =
      user.collection.find((item) => item.card.cardId === row.card5Id)?.card ||
      null;
    const petCard =
      user.collection.find((item) => item.card.cardId === row.petId)?.card ||
      null;
    const arenaCard =
      user.collection.find((item) => item.card.cardId === row.arenaId)?.card ||
      null;
